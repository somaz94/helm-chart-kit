...
| `HCK040` | info | 클레임이 플랫폼이 제공하지 않는 storage class를 요청함 |

경고는 기본적으로 통과하고, `--strict`를 주면 실패합니다. `info`는 둘 중 어느 쪽도 아닙니다. 차트가 잘못됐다는 게 아니라 차트가 자기 클러스터에 대해 갖는 전제조건을 보고하는 것이고, 어떤 엄격도에서도 실패시키지 않습니다. `hck new --platform aws`는 `gp3`를 요청하는데, EKS에서 옳은 클래스이면서 EKS가 만들어주지 않는 클래스입니다. 그래서 `HCK040`이 그 사실을 말하고, 차트는 그대로 통과합니다. `hck list rules`를 실행하면 `hck check`가 쓰는 문구 그대로 같은 표를 볼 수 있습니다.
...
각 규칙은 `off`, `info`, `warn`, `error` 중 하나를 받습니다. 양방향 모두 쓸모가 있습니다. `HCK040: warn`은 클러스터 전제조건을 `--strict`가 막아서는 것으로 바꾸고, 반대로 경고를 `info`로 내리면 그 지적을 보고서에 남기면서도 빌드를 실패시키지는 않습니다. 없는 ID를 적으면 조용히 무시하지 않고 오류를 냅니다. `HCK025`를 적어 두는 목적 자체가 그 지적을 그만 보는 것인데, 철자가 틀려서 계속 보고되는 상태는 규칙이 옳아서 계속 보고되는 상태와 구별되지 않기 때문입니다. `HCK001`만 예외로, 렌더되지 않는 차트에는 더 보고할 것이 없으므로 설정 대상이 아닙니다. 차트가 꺼 둔 규칙은 지적 사항과 함께 출력됩니다. 규칙 절반을 꺼 놓은 차트에서 나온 깨끗한 결과는 보이는 것만큼을 말해 주지 않기 때문입니다.
