...
세 심각도 중 무엇을 고를지는 결국 한 가지 질문입니다. **이건 누구에게 결함인가?** `Error`와 `Warn`은 둘 다 차트가 잘못됐다고 말하며, 얼마나 나쁜가에서만 갈립니다. `Info`는 차트는 맞고 그 바깥의 무언가가 참이어야 한다고 말합니다. hck가 볼 수는 있어도 확인할 수는 없는, 클러스터에 대한 전제조건입니다.

판별 기준은 hck 자신의 출력이 그 지적을 만들어 낼 수 있는가입니다. 여기서는 만들어 냅니다. `hck new --platform aws`가 `persistence.storageClass: gp3`을 쓰고, `HCK040`이 그걸 보고합니다. 이걸 경고로 두면 hck가 만든 차트가 hck 자신의 `--strict`에서 실패하게 되는데, 그건 지적이 아니라 모순입니다. 그러니 hck가 직접 쓰는 것에 대해 발동하는 규칙은 info이거나, 아니면 생성기 쪽의 버그입니다. `--strict`는 info에 대해 절대 실패하지 않으며, 실패하기를 바라는 차트는 그 규칙을 올리면 됩니다.
...
의도적으로 텍스트를 그대로 확인하는 곳은 하나입니다 — helm이 렌더한
출력(`hck check --print`). 그 문자열은 차트의 것이지 hck의 것이 아닙니다.

`hck list`와 `hck sync`도 `check`와 같은 이유로 `--format json`을 갖게 됐습니다.
그러지 않으면 워크플로가 그 표를 파싱해야 했고, 실제로 파서 하나가 깨졌습니다.
카탈로그를 그룹으로 묶으면서 목록의 들여쓰기가 두 칸에서 네 칸으로 바뀌었고
`awk` 패턴 세 개가 같이 나갔습니다. "The JSON contract" 스텝이 필드 이름을
고정하므로, 필드를 지우면 거기서 먼저 실패합니다.
