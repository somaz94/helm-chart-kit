...
// A file that cannot be read is not the same as one that was edited, and
// reporting it as edited would invite "hck sync --write" to overwrite it.
func TestDriftOfReportsAnUnreadableFile(t *testing.T) {
	c := newChart(t, "web")
	hpa, _ := catalog.LookupResource("hpa")
	...
	if d.State != Unreadable || d.Err == nil {
		t.Errorf("State = %s, Err = %v", d.State, d.Err)
	}
	if !AnyDrifted([]Drift{d}) {
		t.Error("an unreadable template is not drift")
	}
}
