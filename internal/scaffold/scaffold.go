...
func overlayHeader(chartName, summary string, install []string, needs []string) string {
	...
	b.WriteString("#\n")
	fmt.Fprintf(&b, "#   helm install %s . %s\n", chartName, strings.Join(install, " "))
	b.WriteString("#\n")
	if len(needs) > 0 {
...
func BuildPlatformValues(data render.Data, resources []catalog.Resource, p catalog.Platform) ([]byte, bool, error) {
	header := overlayHeader(data.ChartName, p.Summary, []string{"-f " + p.ValuesFile()}, p.Needs)
...
func BuildEnvironmentValues(data render.Data, resources []catalog.Resource, e catalog.Environment) ([]byte, bool, error) {
	header := overlayHeader(data.ChartName, e.Summary,
		[]string{"[-f values-<platform>.yaml]", "-f " + e.ValuesFile()}, nil)
