...
// PlatformHeader opens a generated overlay. It says what the file is for,
// because a values file with no context is indistinguishable from one someone
// meant to finish editing.
func platformHeader(p catalog.Platform, chartName string) string {
	...
	b.WriteString("#\n")
	fmt.Fprintf(&b, "#   helm install %s . -f %s\n", chartName, p.ValuesFile())
	b.WriteString("#\n")
...
// BuildPlatformValues assembles a platform overlay for a resource set. It
// returns false when nothing in the chart differs on that platform, so a
// worker chart on GCP does not get a file consisting of a header.
func BuildPlatformValues(data render.Data, resources []catalog.Resource, p catalog.Platform) ([]byte, bool, error) {
	var body strings.Builder
	for _, r := range canonicalOrder(resources) {
		frag, ok, err := render.ResourcePlatformValues(r.Name, p.Name, data)
		if err != nil {
			return nil, false, fmt.Errorf("%s overlay for %s: %w", p.Name, r.Name, err)
		}
		if !ok {
			continue
		}
	...
	return []byte(platformHeader(p, data.ChartName) + "\n" + body.String()), true, nil
}
